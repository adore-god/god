document.addEventListener("DOMContentLoaded", function () {
  ...
  // Map books to Blue Letter Bible URL short codes
  const bookBLBMap = {
    "Genesis": "gen",
    "Exodus": "exo",
    "Leviticus": "lev",
  ...
  // Highlight Bible references in the page
  ...
  document.body.innerHTML = document.body.innerHTML.replace(pattern, function(match, book, chapter, verse){
    return `<span class="bibleref" data-book="${book}" data-chapter="${chapter}" data-verse="${verse}">${match}</span>`;
  });

  // Create tooltip
  const tooltip = document.createElement("div");
  tooltip.style.position = "absolute";
  tooltip.style.background = "#f7f7f7"; // subtle grey
  tooltip.style.color = "#000"; // greyed text
  ...
  document.querySelectorAll(".bibleref").forEach(ref => {
    ...
    ref.addEventListener("mouseenter", async function(e){
      ...
      link.style.color = "#555"; // greyed link text
