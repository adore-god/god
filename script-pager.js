(function() {
    ...
    function injectSeriesLinks() {
        ...
        // If elements aren't ready, retry safely
        if (!labelContainer || !map || !target) {
            setTimeout(injectSeriesLinks, 100);
            return;
        }
        ...
        allLinks.forEach(link => {
            ...
            for (let path in map) {
                const seriesList = Array.isArray(map[path].series) ? map[path].series : [];
        ...
        if (matchedScrollUrls.length === 0) {
            updateSchema(); // Run schema even if no series links found
            return;
        }
        ...
        // --- TRIGGER SCHEMA UPDATE ONLY AFTER INJECTION IS DONE ---
        updateSchema();
    }

    // --- 2. THE SCHEMA UPDATER ---
    function updateSchema() {
        ...
        // Update Series Links (Freshly Injected)
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", injectSeriesLinks);
    } else {
        injectSeriesLinks();
    }
})();
